 ...
 *
 * FIXED (PERFORMANCE): Eliminated memory leak by removing evicted itemsets from
 * seenItemsets tracking. Uses BitSet representation for memory efficiency.
 * Previous: seenItemsets could grow to millions of entries with only k items in heap.
 * Fixed: seenItemsets size = O(k), not O(total_discovered_itemsets).
 */
class TopKHeap {
    private final int k;
    private final int globalMinsup;
    private final PriorityQueue<Itemset> heap;
    // FIXED: Use BitSet instead of Set<String> for memory efficiency
    ...
    public synchronized void insert(Itemset itemset) {
        ...
        // Create copy for set storage (BitSet uses identity equals by default)
        BitSet itemBitsCopy = new BitSet();
        itemBitsCopy.or(itemBits);

        if (seenItemsetBits.contains(itemBitsCopy)) {
            return;  // Already seen this itemset
        }

        if (heap.size() < k) {
            // Heap not full yet - add to both heap and seen set
            heap.offer(itemset);
            ...
            // FIXED: Remove evicted itemset from seen set to prevent memory leak
    ...
    /**
     * Get size of seen itemsets tracking set (for testing/monitoring)
     * Should be O(k), not O(total_discovered_itemsets) after fix
     */
...
class LRUCache<K, V> extends LinkedHashMap<K, V> {
    ...
    // CRITICAL FIX (Issue #5): Use AtomicLong for thread-safe statistics
    // Without this, parallel execution causes lost updates to counters
    ...
    public synchronized V get(Object key) {
        ...
        if (value != null) {
            ...
            cacheMisses.incrementAndGet();  // Atomic increment
        }
        return value;
    }
...
class SafeCache<K, V> {
     ...
     * CRITICAL FIX (Issue #5): Updated to return long (thread-safe AtomicLong)
     */
    public long getCacheHits() {
...
/**
 * CRITICAL FIX (Issue #1): CacheKey for preventing cache collision across conditional databases
 *
 * Problem: Previous implementation used Set<String> itemset as cache key without database context.
 * This caused cache collisions when the same itemset appeared in different conditional databases,
 * leading to incorrect support values being returned from cache.
 *
 * Solution: Composite key combining itemset + database prefix ensures cache entries are
 * specific to their database context.
 *
 * Example of bug this fixes:
 *   Thread A: computes {A,B} in DB|C with support=5, caches result
 *   Thread B: requests {A,B} in DB|D, gets Thread A's cached result (wrong!)
 *   With CacheKey: Two different keys prevent collision
 */
class CacheKey {
    ...
    public CacheKey(Set<String> itemset, Set<String> databasePrefix) {
        ...
        // Pre-compute hash code once (immutable after construction)
    ...
    public boolean equals(Object o) {
        ...
        // Both itemset AND prefix must match for cache hit
...
public class TUFCI {
    ...
    /**
     * CRITICAL FIX (Issue #2): Strict validation mode for numerical stability.
     *
     * When true: Throws exception if distribution normalization is needed
     *            (indicates numerical instability or logic errors)
     * When false: Silently normalizes distributions and continues with warning
     *
     * Default: false (production mode - tolerates small numerical errors)
     * Testing: Should be set to true to detect bugs early
     *
     * Can be enabled via command-line flag: --strict
     */
    private static boolean STRICT_VALIDATION_MODE = false;

    /**
     * CRITICAL FIX (Issue #10): Dynamic cache sizing based on database size and available memory.
     *
     * Problem: Fixed 100K cache can grow to 8GB for large databases (n=10,000 transactions)
     * Each cache entry stores distribution of size O(n), so total memory = O(maxEntries × n × 8 bytes)
     *
     * Solution: Calculate max cache entries based on:
     * - Number of transactions (affects distribution size)
     * - Available JVM memory
     * - Target maximum cache memory (1GB by default)
     *
    ...
    private static int computeMaxCacheEntries(int nTransactions) {
        ...
        // - Distribution array: nTransactions * 8 bytes (double)
        // - Frequentness array: nTransactions * 8 bytes (double)
        // - Transaction probabilities: nTransactions * 8 bytes (double)
        // - Overhead (objects, keys, etc.): ~400 bytes
        long bytesPerEntry = (long) nTransactions * 3 * 8 + 400;

        // Target: Use at most 1GB for cache (or 25% of available memory, whichever is smaller)
        Runtime runtime = Runtime.getRuntime();
        long maxMemory = runtime.maxMemory();
        long availableMemory = maxMemory - (runtime.totalMemory() - runtime.freeMemory());

        // Conservative: Use max 1GB or 25% of available memory
        ...
        // Clamp to reasonable range [1000, 100000]
        int result = (int) Math.max(1000, Math.min(100_000, maxEntries));

        // For very large n, warn user
        if (nTransactions > 5000 && result < 10_000) {
            System.err.printf(
                "WARNING: Large database (n=%d) with limited cache (%d entries, ~%d MB).%n" +
                "         Consider increasing JVM heap size for better performance.%n",
                nTransactions, result, (result * bytesPerEntry) / 1_000_000
            );
        }
     ...
     *
     * FIXED (CRITICAL): Thread-safe implementation using ThreadLocal to eliminate
     * race conditions in parallel execution. Each thread maintains its own cache,
     * preventing mismatched itemset-result pairs that could cause false positives.
     */
    private static class DistributionCache {
        ...
        /**
         * Thread-local cache entry holding distribution cache state
         */
        private static class CacheEntry {
            SupportResult lastResult = null;
            Set<String> lastItemset = null;
            double[] lastTransProbs = null;
            Set<String> lastDatabasePrefix = null;  // Track which database this cache belongs to
        }

        static void set(SupportResult result, Set<String> itemset, double[] transProbs, Set<String> databasePrefix) {
            ...
            entry.lastDatabasePrefix = new HashSet<>(databasePrefix);  // Store database context
        }
        ...
        static boolean canReuse(Set<String> newItemset, Set<String> databasePrefix) {
            ...
            // CRITICAL: Check database context first (fixes ThreadLocal cache corruption bug)
            // The cached distribution is only valid for the same conditional database
            if (entry.lastDatabasePrefix == null || !entry.lastDatabasePrefix.equals(databasePrefix)) {
                return false;  // Different database context - cannot reuse
            }
     ...
     *
     * OPTIMIZATION (Issue #11): Binary search instead of linear search
     * Time complexity: O(log n) instead of O(n)
     *
     * Note: frequentness is monotone decreasing: frequentness[i] >= frequentness[i+1]
     * We want largest i where frequentness[i] >= tau
     */
    private static int findProbabilisticSupport(double[] frequentness, double tau) {
        ...
        while (left <= right) {
            ...
            if (frequentness[mid] >= tau) {
                ...
                right = mid - 1;  // mid is too small, search left half
            }
        }
     ...
     *
     * FIXED (CRITICAL): Corrected zero probability handling to enforce extension property.
     * When prevProb=0, newProb MUST be 0 (can't have {A,B} if you don't have A).
     */
    private static double[] refineDistribution(
        ...
        // CRITICAL FIX (Issue #8): Stricter epsilon for numerical stability
        // Increased from 1e-10 to 1e-9 to reduce risk of division by very small numbers
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (prevTransProbs[i] > EPSILON) {
                ...
                // CRITICAL FIX (Issue #8): Add explicit NaN check
                // Could occur if division produces NaN due to extreme values
                ...
                // FIXED: Extension property: P(prev) = 0 ⟹ P(new) = 0
                // If previous itemset not in transaction, extension can't be either
        ...
        if (Math.abs(sum - 1.0) > 1e-6) {
            ...
            if (STRICT_VALIDATION_MODE) {
                // STRICT MODE: Fail fast to expose underlying bugs
                // (cache corruption, logic errors, or extreme numerical instability)
                ...
                // NORMAL MODE: Normalize and continue with warning
    ...
    private static SupportResult computeSupport(
        ...
        // Get database prefix for cache context validation
     ...
     *
     * Where:
     *   - E[sup(X)] = Σ P(X ⊆ Ti) for all transactions Ti
     *   - σk = current k-th largest support (topKHeap.getMinSupport())
     *   - τ = probability threshold
     *
     * Performance Impact:
     * -------------------
     * - Time: O(n × |X|) vs O(n²) for full binomial convolution
     * - Savings: 50-100x speedup on pruned candidates
     * - Expected pruning rate: 30-50% of candidates on dense datasets
     *
     * CRITICAL CORRECTNESS NOTE:
     * The pruning threshold MUST include the τ factor: σk × τ
     * Using just σk would be mathematically INCORRECT and cause false negatives!
     *
     * @param itemset The itemset to evaluate
     * @param database The uncertain database (may have prefix)
     * @param tau The probability threshold τ ∈ (0, 1]
     * @param topKHeap The top-k heap containing current best itemsets
    ...
    private static Double computeExpectedSupportUpperBound(
            ...
            double tau,
            TopKHeap topKHeap,
            int globalMinsup) {  // FIX #7: Add globalMinsup parameter

        // FIX #7: Always allow computation to proceed - we'll use globalMinsup for pruning
        // (removed size check that prevented early pruning)
        if (globalMinsup <= 0) {
            return null;  // Don't prune if no minimum support
        }
        ...
        // Compute E[sup(X)] = Σ P(X ⊆ Ti) for all transactions
        ...
        for (Map.Entry<Integer, Map<String, Double>> entry :
            ...
            // Convert back to probability space and accumulate
            double prob = NumericalStability.expSafe(logTotalProb);
            expectedSupport += prob;
        }

        // CRITICAL: Apply correct pruning threshold with τ factor!
        // Theorem: If E[sup(X)] < σ × τ, then SupD(X, τ) < σ
        //
        // FIX #7: Use fixed globalMinsup instead of dynamic topKHeap.getMinSupport()
        // This ensures correctness when threshold changes during parallel execution.
        // Using globalMinsup is conservative (may prune less) but always safe.
        double minExpectedSupport = globalMinsup * tau;

        if (expectedSupport < minExpectedSupport) {
            return null;  // Prune: guaranteed SupD(X, τ) < globalMinsup
        }

        // Pass threshold - return expected support for potential future use
    ...
    private static boolean checkClosure(
        ...
        if (frequentCandidates.size() >= MIN_EXTENSIONS_FOR_PARALLEL) {
            ...
            for (String item : frequentCandidates) {
                ...
                // CRITICAL FIX (Issue #1): Use CacheKey with database prefix to prevent cache collision
    ...
    /**
     * PRIORITY P11 OPTIMIZATION: Parallel Candidate Generation Configuration
     * Controls when to use parallel streams for top-level 1-itemset exploration
     */
    private static final int MIN_CANDIDATES_FOR_PARALLEL_GENERATION = 4;  // Min 1-itemsets to parallelize
    ...
    // ========================================================================
    // DEAD CODE REMOVED (Issue #6): MiningTask class (73 lines)
    // This was the OLD parallel mining implementation that has been replaced by
    // MiningTaskCollect. It was never reachable from the main execution path.
    // ========================================================================
    ...
    // ========================================================================
    // BUGFIX: MINING TASK FOR COLLECTION MODE (Fixes Bug #1 and #2)
    // ========================================================================

    /**
     * RecursiveAction for parallel mining in COLLECTION mode
     *
     * CRITICAL FIX for Bugs #1 and #2:
     * - Bug #1: MiningTask was not collecting itemsets into allDiscoveredItemsets
     * - Bug #2: Itemsets from parallel branches were missing from Phase 4 verification
     *
     * This class is specifically for the two-phase mining approach (P1 fix):
     * - Collects ALL itemsets without inline closure checking
     * - Phase 4 performs closure verification on collected itemsets
     * - Calls mineRecursiveCollect (not mineRecursive)
     *
     * Difference from MiningTask:
     * - MiningTask: Checks closure inline, for old approach
     * - MiningTaskCollect: Defers closure to Phase 4, for P1 fix
     */
    private static class MiningTaskCollect extends RecursiveAction {
        ...
        private final List<Itemset> allDiscoveredItemsets;  // CRITICAL: Added parameter
        ...
        MiningTaskCollect(ExtensionCandidate ext, UncertainDatabase database, TopKHeap topk,
            ...
            this.allDiscoveredItemsets = allDiscoveredItemsets;  // CRITICAL
        }
        ...
        protected void compute() {
            ...
            // CRITICAL FIX: Collect itemset WITHOUT inline closure checking
            // Closure verification happens in Phase 4
            ...
            allDiscoveredItemsets.add(itemsetObj);  // CRITICAL: Add to collection
            ...
            if (ext.support >= currentThreshold) {
                ...
                if (!condDb.getTransactions().isEmpty()) {
                    // CRITICAL FIX: Call mineRecursiveCollect (not mineRecursive)
                    mineRecursiveCollect(
                        ...
                        depth + 1,
                        parallel,
                        allDiscoveredItemsets  // CRITICAL: Pass collection forward
                    );
                }
            }
        }
    ...
    private static void mineRecursiveCollect(
        ...
        if (parallel && candidateItems.size() >= MIN_EXTENSIONS_FOR_PARALLEL) {
            ...
            // PERF #1 FIX: Store itemsets to avoid redundant creation
            ...
            for (int i = 0; i < candidateItems.size(); i++) {
                ...
                taskIndexToItemset.put(i, newItemset);  // PERF FIX: Store for reuse
            }
            ...
            for (int i = 0; i < results.size(); i++) {
                ...
                if (result.supT >= minsup) {
                    // PERF #1 FIX: Reuse itemset instead of recreating
        ...
        if (parallel && extensions.size() >= MIN_EXTENSIONS_FOR_PARALLEL &&
            ...
            if (estimatedWork >= TASK_WORK_THRESHOLD) {
                ...
                // BUGFIX: Use MiningTaskCollect (not MiningTask) for collection mode
                ...
                for (ExtensionCandidate ext : extensions) {
                    ...
                    // CRITICAL FIX: Use MiningTaskCollect with allDiscoveredItemsets parameter
    ...
    private static int verifyClosureProperty(List<Itemset> allItemsets, boolean verbose) {
        ...
        for (int i = 0; i < sortedItemsets.size(); i++) {
            ...
            for (int j = i - 1; j >= 0; j--) {
                ...
                //
                // FIX #3 VERIFICATION: Support values stored in Itemset objects are correct
                // for comparison even though they were computed in conditional databases.
                // Rationale: Sup_T(X,τ) = count of (original) transactions containing X
                // with P(X⊆t) ≥ τ. This count is consistent across all database contexts.
    ...
    // ========================================================================
    // DEAD CODE REMOVED (Issue #6): mineRecursive() and processExtension() methods
    // ========================================================================
    // Removed 320 lines of unreachable code:
    //   - mineRecursive() method (211 lines): OLD pattern growth implementation
    //   - processExtension() method (109 lines): OLD extension processing logic
    //
    // These methods were replaced by mineRecursiveCollect() and processExtensionCollect()
    // but the old versions were never removed. The new two-phase approach (collect all
    // itemsets, then verify closure in post-processing) is now the only execution path.
    // ========================================================================
    ...
    public static List<Itemset> runTUFCI(
        ...
        // CRITICAL FIX (Issue #7): Explicit k=0 handling with clear behavior
        if (k == 0) {
            if (verbose) {
        ...
        if (verbose) {
            System.out.printf("Cache configuration: max %d entries (~%d MB estimated)%n",
                maxCacheEntries,
                (maxCacheEntries * (database.getNTrans() * 3 * 8 + 400)) / 1_000_000);
        }
        ...
        if (verbose) {
            ...
            if (parallel) {
                ...
                if (F1.size() >= MIN_CANDIDATES_FOR_PARALLEL_GENERATION) {
                    System.out.println("  PRIORITY P11: Parallel candidate generation enabled");
        ...
        // PRIORITY P11: Use thread-safe list for parallel candidate generation
        List<Itemset> allDiscoveredItemsets = Collections.synchronizedList(new ArrayList<>(F1));

        // PRIORITY P11: Parallel Candidate Generation
        // Parallelize the top-level mining loop to explore multiple branches concurrently
        // This is different from P5A (internal parallel support computation)
        // Here we parallelize the exploration of different 1-itemset branches
        boolean useParallelGeneration = parallel && F1.size() >= MIN_CANDIDATES_FOR_PARALLEL_GENERATION;

        if (useParallelGeneration) {
            // PRIORITY P11: Parallel candidate generation using parallel streams
            // Each thread explores a different branch of the pattern growth tree
            //
            // FIX #1: Snapshot threshold before parallelization for determinism
            // Using takeWhile() with mutable topk.getMinSupport() causes non-deterministic
            // results because threads may see different threshold values during execution.
            ...
            F1.parallelStream()
                ...
                .forEach(itemset -> {
                    ...
                    mineRecursiveCollect(
                        ...
                        cache,
                        minsup,
                        false,  // Disable verbose for parallel to avoid interleaved output
                        1,
                        parallel,  // Still use parallel for internal operations
                        allDiscoveredItemsets
                    );
                });
        } else {
            // Sequential candidate generation (original approach)
            for (Itemset itemset : F1) {
    ...
    public static UncertainDatabase loadUncertainDatabase(String filename)
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filename))) {
            ...
            // Auto-detect format: check if first line is metadata or transaction data
            // Metadata format: exactly 2 integers (no colons)
            // Transaction format: tid followed by item:prob pairs (contains colons)
            ...
            if (parts.length == 2 && !line.contains(":")) {
                try {
                    ...
                    isMetadataLine = true;
                    // Metadata detected - skip this line (values not used in algorithm)
                    line = reader.readLine();
                } catch (NumberFormatException e) {
                    // Not metadata, process as transaction
                    isMetadataLine = false;
                }
            }

            // Process transactions (starting from first line if no metadata, or second line if metadata)
            do {
                if (line == null) break;
                line = line.trim();
    ...
    public static void main(String[] args) {
        ...
        // Check for --strict flag (can appear anywhere in arguments)
        for (String arg : args) {
            if ("--strict".equals(arg)) {
                STRICT_VALIDATION_MODE = true;
                System.err.println("⚠️  STRICT VALIDATION MODE ENABLED");
                System.err.println("   Algorithm will fail fast on numerical instability");
                break;
            }
        }

        // Parse command line arguments (skip --strict if present)
        String inputFile = args[0];
        int minsup = (args.length > 1 && !args[1].equals("--strict")) ? Integer.parseInt(args[1]) : 2;
        double tau = (args.length > 2 && !args[2].equals("--strict")) ? Double.parseDouble(args[2]) : 0.7;
        int k = (args.length > 3 && !args[3].equals("--strict")) ? Integer.parseInt(args[3]) : 5;
        boolean parallel = (args.length > 4 && !args[4].equals("--strict")) ? Boolean.parseBoolean(args[4]) : false;
