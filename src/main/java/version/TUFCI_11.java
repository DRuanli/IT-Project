...
public class TUFCI {
    ...
    private static Double computeExpectedSupportUpperBound(
        ...
        // Cannot prune without threshold from heap
        if (topKHeap.size() == 0) {
            return null;  // Don't prune, let normal computation proceed
        }
    ...
    public static List<Itemset> runTUFCI(
        ...
        if (useParallelGeneration) {
            ...
            F1.parallelStream()
                .takeWhile(itemset -> itemset.getSupport() >= topk.getMinSupport())
                .forEach(itemset -> {
                    if (verbose) {
