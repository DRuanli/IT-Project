...
public class TUFCI {
    ...
    private static double[] refineDistribution(
        ...
        // Small epsilon for numerical stability (to handle floating-point errors)
        ...
        if (Math.abs(sum - 1.0) > 1e-6) {
            System.err.printf("WARNING: Distribution sum = %.10f (expected 1.0). " +
                "Normalizing to fix numerical errors.%n", sum);
    ...
    private static class MiningTask extends RecursiveAction {
        ...
        private final List<Itemset> allDiscoveredItemsets;  // FIX #2: Add collection
        ...
        MiningTask(ExtensionCandidate ext, UncertainDatabase database, TopKHeap topk,
                   ...
                   boolean verbose, int depth, boolean parallel, List<Itemset> allDiscoveredItemsets) {
            this.ext = ext;
            ...
            this.allDiscoveredItemsets = allDiscoveredItemsets;  // FIX #2
        }
        ...
        protected void compute() {
            ...
            // FIX #2: Remove premature closure checking from parallel tasks
            // Collect itemset for Phase 4 closure verification
            Itemset itemsetObj = Itemset.fromStringSet(
                ext.newItemset, ext.support, ext.probability, database.getItemCodec()
            );
            itemsetObj.setClosed(false);  // Mark as unchecked - Phase 4 will verify
            ...
            if (ext.support >= currentThreshold) {
                ...
                if (!condDb.getTransactions().isEmpty()) {
                    mineRecursive(
                        ...
                        depth + 1,
                        parallel,
                        allDiscoveredItemsets  // FIX #2: Pass collection
                    );
                }
            }
        }
    ...
    private static void mineRecursive(
        ...
        if (parallel && promisingItems.size() >= MIN_EXTENSIONS_FOR_PARALLEL) {
            ...
            // PRIORITY P10: Apply ESUB pruning before creating parallel tasks
            // Reduces task creation overhead and parallel scheduling cost
            int taskIndex = 0;
            for (String item : promisingItems) {
                Set<String> newItemset = new HashSet<>(prefix);
                newItemset.add(item);

                // PRIORITY P10: ESUB Pruning - Check expected support upper bound
                // before spawning expensive parallel task
                Double expectedSupport = computeExpectedSupportUpperBound(
                    newItemset, database, tau, topk, globalMinsup  // FIX #7
                );

                if (expectedSupport == null) {
                    // Pruned: E[sup(X)] < σk × τ ⟹ SupD(X, τ) < σk
                    // Skip creating parallel task for this itemset
                    continue;
                }
                ...
                taskIndexToItem.put(taskIndex, item);
                taskIndexToItemset.put(taskIndex, newItemset);  // PERF FIX: Store for reuse
                taskIndex++;
            }
            ...
            for (String item : promisingItems) {
                ...
                // PRIORITY P10: ESUB Pruning - Check expected support upper bound
                // before expensive O(n²) distribution computation
                Double expectedSupport = computeExpectedSupportUpperBound(
                    newItemset, database, tau, topk, globalMinsup  // FIX #7
                );
                ...
                if (expectedSupport == null) {
                    // Pruned: E[sup(X)] < σk × τ ⟹ SupD(X, τ) < σk
                    // Skip expensive O(n²) binomial convolution
                    continue;
                }
            ...
            if (estimatedWork >= TASK_WORK_THRESHOLD) {
                ...
                for (ExtensionCandidate ext : extensions) {
                    ...
                    tasks.add(new MiningTask(ext, database, topk, globalMinsup,
                                            tau, cache, verbose, depth, parallel, allDiscoveredItemsets));
                }
    ...
    private static void processExtension(
        ...
        // FIX #2: Remove premature closure checking
        // Closure verification will be done in Phase 4 after all itemsets are discovered
        // This ensures mathematical correctness: X is closed ⟺ ∀Y ⊃ X: Sup(Y) < Sup(X)
        // (Cannot verify closure until ALL supersets Y are known)
        ...
        if (ext.support >= currentThreshold) {
            ...
            if (!condDb.getTransactions().isEmpty()) {
                mineRecursive(
                    ...
                    depth + 1,
                    parallel,
                    allDiscoveredItemsets  // FIX #2: Pass collection for Phase 4
                );
            }
        }
    }
