...
public class TUFCI {
    ...
    private static int verifyClosureProperty(List<Itemset> allItemsets, boolean verbose) {
        ...
        // For each itemset, check if it has any proper superset with support >= itself
        for (Itemset itemset : allItemsets) {
            boolean isClosed = true;

            // Check against all other itemsets
            for (Itemset other : allItemsets) {
                if (other == itemset) {
                    continue;  // Skip self-comparison
                }

                // Check if 'other' is a proper superset of 'itemset'
                // AND has support >= itemset's support
                if (other.getSupport() >= itemset.getSupport() &&
                    other.getItems().containsAll(itemset.getItems()) &&
