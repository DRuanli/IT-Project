...
class UncertainDatabase {
    private final Map<Integer, Map<String, Double>> transactions;
    private final int nTrans;
    private final Set<String> items;
    // PRIORITY 6A: Use BitSet for efficient set operations and memory optimization
    private final Map<String, BitSet> invertedIndex;
    private final Set<String> prefix;  // Items that were projected out in conditional DB
    private final Map<Integer, Double> prefixProbs;  // P(prefix ⊆ t_i) for each transaction
    ...
    public UncertainDatabase getConditionalDb(Set<String> newPrefix) {
        Map<Integer, Map<String, Double>> condTrans = new HashMap<>();
        Map<Integer, Double> newPrefixProbs = new HashMap<>();

        // Merge old prefix with new prefix
        Set<String> fullPrefix = new HashSet<>(this.prefix);
        ...
        for (Map.Entry<Integer, Map<String, Double>> entry : transactions.entrySet()) {
            ...
            // Check if transaction contains newPrefix items
            if (trans.keySet().containsAll(newPrefix)) {
                // Compute probability of newPrefix in this transaction
                ...
                double oldPrefixProb = this.prefixProbs.getOrDefault(tid, 1.0);
                ...
                for (Map.Entry<String, Double> itemEntry : trans.entrySet()) {
                    if (!fullPrefix.contains(itemEntry.getKey())) {
                        projected.put(itemEntry.getKey(), itemEntry.getValue());
                    }
                }

                // Store transaction (even if projected is empty - prefix-only transactions matter)
                condTrans.put(tid, projected);
                newPrefixProbs.put(tid, combinedPrefixProb);
            }
        }

        // PRIORITY P6B: Pass ItemCodec to conditional database constructor
        return new UncertainDatabase(condTrans, fullPrefix, newPrefixProbs, this.itemCodec);
    }
...
public class TUFCI {
    ...
    /**
     * PHASE 4 of P1 FIX: Verify closure property for all discovered itemsets
     * PRIORITY 5A (NEW): Incremental closure checking with sorted ordering for 5-10x speedup
     *
     * Mathematical correctness: X is closed ⟺ ∀Y ⊃ X: Sup(Y) < Sup(X)
     *
     * This post-processing phase checks each itemset against all other
     * discovered itemsets to determine if it's truly closed.
     *
    ...
    private static void processExtension(
        ...
        if (ext.support >= currentThreshold) {
            ...
            // PRIORITY P7A: Phase 1 - Transaction Count Pruning
            // If condDb has fewer transactions than the current threshold,
            // no itemset in condDb can have support >= currentThreshold
            // because support(X) <= |T_X| where T_X is the conditional database
    ...
    public static void main(String[] args) {
        ...
        if (args.length < 1) {
            ...
            System.err.println("  java TUFCI mydata.txt 3 0.8 10");
            System.exit(1);
        }
